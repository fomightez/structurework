#!/usr/bin/env python
# pdb_ids_to_stats_and_info_df.py
__author__ = "Wayne Decatur" #fomightez on GitHub
__license__ = "MIT"
__version__ = "0.1.0"


# pdb_ids_to_stats_and_info_df.py by Wayne Decatur
# ver 0.1
#
#*******************************************************************************
...
#
#
# PURPOSE: Takes a file of PDB id codes listed separately each on a single line
# and gets some statistics and information for each and puts it into a summary 
# Pandas dataframe with a row in the dataframe for each PDB code. 
...
#
#
# Developed by adapting backbone of `pdbsum_stats_and_info_adpated_example` to 
...
#-----------------------------------
# python pdb_ids_to_stats_and_info_df.py FILE_LISTING_PDB_CODES
#-----------------------------------
# Issue `pdb_ids_to_stats_and_info_df.py -h` for details.
# 
...
df_save_as_name = 'stats_and_info_for_PDBids_pickled_df.pkl' #name for pickled 
#dataframe file
...
def pdb_ids_to_stats_and_info_df(file_name, return_df = True, pickle_df=True):
    '''
    Main function of script. 
    PDBsum information for PDB codes provided in a file converted to 
    an easy-to-viewPandas dataframe.
    ...
    # READ IN THE PDB CODES FROM THE TEXT FILE TO A PYTHON LIST:
    #--------------------------------------------------------------------------#
    pdb_codes_df = pd.read_csv(file_name, names=["pdb_ids"])




    # MAKE DATAFRAMES FOR EACH PDB CODE:
    #--------------------------------------------------------------------------#
    sys.stderr.write("Parsing details from PDBsum ...\n")
    dfs = []
    with suppress_stdout_stderr():
        for pdb_code in pdb_codes_df.pdb_ids:
            dfs.append(pdbsum_stats_and_info_adpated_example(pdb_code,
                pickle_df=False))
            sys.stderr.write("\n")
            time.sleep(0.3)
    ...
    sys.stderr.write("Information for specified structures read and "
...
if __name__ == "__main__" and '__file__' in globals():
    ...
    import argparse
    parser = argparse.ArgumentParser(prog='pdb_ids_to_stats_and_info_df.py',
        description="pdb_ids_to_stats_and_info_df.py \
        Takes a file of PDB id codes listed separately each on a single line \
        and gets some statistics and information for each and puts it into a \
        summary Pandas dataframe with a row in the dataframe for each PDB code.\
         Optionally, it can \
        ...
        **** Script by Wayne Decatur   \
        (fomightez @ github) ***")
    parser.add_argument("file_name", help="file containing the list of PDB code\
        identifiers each on a separate line.",
        metavar="PDB_CODE")
