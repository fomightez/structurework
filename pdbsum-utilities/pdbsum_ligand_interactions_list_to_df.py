...
# pdbsum_ligand_interactions_list_to_df.py by Wayne Decatur
# ver 0.1
#
#*******************************************************************************
...
#
#
# PURPOSE: Takes a list of interactions involving a ligand as text from PDBsum 
# and brings it into Python as a dataframe and saves a 
...
#
#
# Developed by adapting `pdbsum_prot_interactions_list_to_df.py` to handle the 
# ligand interaction data. The list of interactions PDBsum has for ligands
# under 'ligands' tab at PDBsum is structured similar to the list of 
# interactions available from the 'prot-prot' tab for each combination of 
# protein-protein interaction and so it makes sense to use that as the backone.
#
#
#
...
#-----------------------------------
# python pdbsum_ligand_interactions_list_to_df.py DATA_FILE_NAME
#-----------------------------------
# Issue `pdbsum_ligand_interactions_list_to_df.py -h` for details.
# 
...
# df = pdbsum_ligand_interactions_list_to_df("test_data.txt")
# df
#
#
...
#ligand tab in PDBsum
#http://www.ebi.ac.uk/thornton-srv/databases/cgi-bin/pdbsum/RunLigplot.ps?pdb=4tz4&t=&file=ligplot01_01
#https://www.ebi.ac.uk/thornton-srv/databases/cgi-bin/pdbsum/GetPage.pl?pdbcode=4LXZ&template=ligands.html&l=1.1
#https://www.ebi.ac.uk/thornton-srv/databases/cgi-bin/pdbsum/GetLigInt.pl?pdb=4lxz&ligtype=03&ligno=01
#
#*******************************************************************************
#
...
df_save_as_name = 'ligand_int_pickled_df.pkl' #name for pickled dataframe file
...
def type_2_key_and_text_to_value(text,title_underline):
    ...
    Note that turns out while protein-protein interfaces seem limited to 
    `<-->`, for ligand interactions also seeing the following as separator 
    between the two atoms in the list:
        -->
        <--
        ---
    (In fact latter, seems more common in this list than the protein-protein 
    one.)
    HOWEVER, cannot remove here because those match parts of the 
    'Atom set column indicator lines' (see below). Have to deal with them later
    and doing when remove those 'Atom set column indicator lines' (see below).
    '''
...
def pdbsum_ligand_interactions_list_to_df(data_file, return_df = True, 
    ...
    ligand_code = raw_data_txt.split(pdb_code_delimiter,1)[1].split()[2].strip()
    ...
    # start of each section. (note, these are slightly different than very 
    # similar line in prot-prot interactions list text)
    # Also, once the 'Atom set column indicator line' are gone it is safe to 
    # remove the following between the two atoms in the list: `-->`,`<--`, `---`
    ...
    for section in sections:
        ...
        for k,v in section.items():
            if col_indic in v:
                fixed_section[k] = v.split(col_indic)[1].strip().replace(
                    '-->','').replace('<--','').replace('---','')
    ...
    #df.columns = df'Hydrogen bonds': 'Atom Atom Res  Res              Atom Atom Res  Res\.columns.map(' '.join)
...
def main():
    ...
    pdbsum_ligand_interactions_list_to_df(data_file,**kwargs)
...
if __name__ == "__main__" and '__file__' in globals():
    ...
    import argparse
    parser = argparse.ArgumentParser(prog='pdbsum_ligand_interactions_list_to_df.py',
        description="pdbsum_ligand_interactions_list_to_df.py \
        Takes the text of list of ligand interactions from PDBsum & brings it \
